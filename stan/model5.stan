data {
  ...
  int<lower=1> L; // num boroughs
  array[N] int<lower=1,upper=L> l_index; // borough index
...
parameters {
  ...
  vector[L] beta_l; // borough-level fixed effects
  vector[L] beta_l_z; // interaction effect of borough with treatment z
...
model {
  ...
  vector[N] mu;
  for (n in 1:N) {
    mu[n] = beta0 + beta_i[i_index[n]] + beta_j[j_index[n]] + beta_l[l_index[n]] + beta_r[r_index[n]] + theta * z[n] + beta_l_z[l_index[n]] * z[n];
  }

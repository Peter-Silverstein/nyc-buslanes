...
```{r}
# Getting accurate route geoms
route_shapes_filt_sf <- route_shapes_sf %>% 
  group_by(Route_ID) %>%
  top_n(1, Shape_Length)
```
...
route_shapes_proj <- st_transform(route_shapes_filt_sf, 2263)  
...
route_length <- merged_routes %>%
  select(Route_ID, total_length_ft) %>%
  mutate(route_length_miles = as.numeric(total_length_ft/5280)) %>%
  select(!total_length_ft)

df_complete <- df_complete %>%
  left_join(route_length, by = "Route_ID")
...
df_complete <- df_complete %>%
  ...
  mutate(lane_perc = lane_miles / route_length_miles * 100)

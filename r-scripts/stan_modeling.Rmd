...
# Stan setup
N2 <- as.numeric(nrow(df))
y2 <- as.numeric(df$average_speed)
z2 <- as.numeric(df$lane_perc)
I2 <- max(as.numeric(df$year))
i_index2 <- as.numeric(df$year)
J2 <- max(as.numeric(df$month))
j_index2 <- as.numeric(df$month)
L2 <- max(as.numeric(df$trip_type))
l_index2 <- as.numeric(df$trip_type)
R2 <- max(as.numeric(df$route_num))
r_index2 <- as.numeric(df$route_num)
...
```{r}
fit4_2$summary(variables = c("beta0", "theta", "sigma", "sigma_r", "beta_i", "beta_j", "beta_l"))
```

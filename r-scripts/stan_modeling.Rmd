---
title: "stan_modeling"
author: "Peter Silverstein"
date: "`r Sys.Date()`"
output: html_document
---

```{r setup, include=FALSE}
library(tidyverse)
library(ggplot2)
library(cmdstanr)
library(rstanarm)
```

```{r}
# Load in data
df <- read_csv("../data/final_modeling_data.csv")
df <- df %>%
  filter(year > 2021) %>%
  mutate(year = year - 2021) %>%
  filter(peak == 1)
```

```{r}
# Model 1: simple regression of continuous treatment on outcome variable
set.seed(55)

fit1 <- stan_glm(average_speed ~ lane_perc,
                  data = df,
                  refresh = FALSE,
                  cores = 4)

summary(fit1, digits = 3)

# Interpretation: 

# R-Hat: all at 1.0
```

```{r}
# Model 2: regression of continuous treatment on outcome variable, plus covariates
set.seed(55)

fit2 <- stan_glm(average_speed ~ lane_perc + 
                   as.factor(borough) + 
                   as.factor(year) + 
                   as.factor(month),
                  data = df,
                  refresh = FALSE,
                  cores = 4)

summary(fit2, digits = 3)

# Interpretation: 

# R-Hat: all at 1.0
```

```{r}
# Model 3: regression of continuous treatment on outcome variable, plus covariates, plus varying treatment effect by borough
set.seed(55)

fit3 <- stan_glm(average_speed ~ lane_perc + 
                   lane_perc:as.factor(borough) + 
                   as.factor(borough) + 
                   as.factor(year) + 
                   as.factor(month),
                  data = df,
                  refresh = FALSE,
                  cores = 4)

summary(fit3, digits = 3)

# Interpretation: 
## M Effect: 1.28 -> 1.3
## BX Effect: 1.28 + 0.28 = 1.56 -> 1.6
## BK Effect: 1.28 + 2.03 = 3.31 -> 3.3
## Q Effect: 1.28 - 0.75 = 0.53 -> 0.5
## S Effect: 1.28 + 1.79 = 3.07-> 3.1

# R-Hat: all at 1.0
```

```{r}
# Stan setup
N <- as.numeric(nrow(df))
y <- as.numeric(df$average_speed)
z <- as.numeric(df$lane_perc)
I <- max(as.numeric(df$year))
i_index <- as.numeric(df$year)
J <- max(as.numeric(df$month))
j_index <- as.numeric(df$month)
L <- max(as.numeric(df$trip_type))
l_index <- as.numeric(df$trip_type)
M <- max(as.numeric(df$borough))
m_index <- as.numeric(df$borough)
R <- max(as.numeric(df$route_num))
r_index <- as.numeric(df$route_num)

data <- list(
  N = N,
  y = y,
  z = z,
  I = I,
  i_index = i_index,
  J = J,
  j_index = j_index,
  L = L,
  l_index = l_index,
  M = M,
  m_index = m_index,
  R = R,
  r_index = r_index
)
```


```{r}
# Model 4: regression of continuous treatment on outcome variable, plus covariates, plus route_id mlm
model4 <- cmdstan_model("../stan/model4.stan")

set.seed(55)

fit4 <- model4$sample(data = data, parallel_chains = 4, refresh = 0)
# fit4 <- model4$optimize(data = data, refresh = 0)

# Interpretation: average speed across the sample was 9.17320279 mph, an increase of 1 in lane_perc (+ 1 percentage point additional lane coverage) is associated with an increase of 0.45008774 mph on average

# R-Hat: all well below 1.1
```

```{r}
fit4$save_object(file = "../stan_models/fit4_saved_minlength.rds")
fit4$summary(variables = c("beta0", "theta", "sigma", "sigma_r", "beta_i", "beta_j", "beta_m"))
```

```{r}
# Model 5: regression of continuous treatment on outcome variable, plus covariates, plus route_id mlm, plus borough-level treatment interaction
model5 <- cmdstan_model("../stan/model5.stan")

set.seed(55)

fit5 <- model5$sample(data = data, parallel_chains = 4, refresh = 0)
# fit5 <- model5$optimize(data = data, refresh = 0)

# Interpretation: average speed across the sample for Manhattan was 6.662 mph, an increase of 1 in lane_perc (+ 1 percentage point additional lane coverage) is associated with an increase of 1.1 mph on average

# R-Hat: all at 1.0
```

```{r}
fit5$save_object(file = "../stan_models/fit5_saved_minlength.rds")
fit5$summary(variables = c("beta0", "theta", "beta_l_z", "sigma", "sigma_r", "beta_i", "beta_j"))
```

Further modeling thoughts
1. set up a baseline treatment indicator
2. what does the borough-level effect tell us?
3. run a version where first observation has 0, how do the coefs change (sensitivity analysis)

```{r}
# Sensitivity Analysis
starts_at_0 <- df %>%
  filter(year == 1, month == "01", lane_perc < 1) %>%
  select(Route_ID)

df2 <- df %>%
  filter(Route_ID %in% starts_at_0$Route_ID)

# Stan setup
N2 <- as.numeric(nrow(df2))
y2 <- as.numeric(df2$average_speed)
z2 <- as.numeric(df2$lane_perc)
I2 <- max(as.numeric(df2$year))
i_index2 <- as.numeric(df2$year)
J2 <- max(as.numeric(df2$month))
j_index2 <- as.numeric(df2$month)
L2 <- max(as.numeric(df2$trip_type))
l_index2 <- as.numeric(df2$trip_type)
M2 <- max(as.numeric(df2$borough))
m_index2 <- as.numeric(df2$borough)
R2 <- max(as.numeric(df2$route_num))
r_index2 <- as.numeric(df2$route_num)

data2 <- list(
  N = N2,
  y = y2,
  z = z2,
  I = I2,
  i_index = i_index2,
  J = J2,
  j_index = j_index2,
  L = L2,
  l_index = l_index2,
  M = M2,
  m_index = m_index2,
  R = R2,
  r_index = r_index2
)
```

```{r}
# Model 4 Again

# Model 4: regression of continuous treatment on outcome variable, plus covariates, plus route_id mlm
model4 <- cmdstan_model("../stan/model4.stan")

set.seed(55)

fit4_2 <- model4$sample(data = data2, parallel_chains = 4, refresh = 0)

# Interpretation: average speed across the sample was 9.17320279 mph, an increase of 1 in lane_perc (+ 1 percentage point additional lane coverage) is associated with an increase of 0.45008774 mph on average

# R-Hat: all well below 1.1
```

```{r}
fit4_2$summary(variables = c("beta0", "theta", "sigma", "sigma_r", "beta_i", "beta_j", "beta_m"))
```

```{r}
# Model 5 Again

# Model 5: regression of continuous treatment on outcome variable, plus covariates, plus route_id mlm, plus borough-level treatment interaction
model5 <- cmdstan_model("../stan/model5.stan")

set.seed(55)

fit5_2 <- model5$sample(data = data2, parallel_chains = 4, refresh = 0)

# Interpretation: average speed across the sample was 9.17320279 mph, an increase of 1 in lane_perc (+ 1 percentage point additional lane coverage) is associated with an increase of 0.45008774 mph on average

# R-Hat: all well below 1.1
```

```{r}
fit5_2$summary(variables = c("beta0", "theta", "beta_l_z", "sigma", "sigma_r", "beta_i", "beta_j"))
```

```{r}
df_sum <- df %>%
  group_by(trip_type) %>%
  summarize(
    covg = mean(lane_perc),
    covg_max = max(lane_perc),
    covg_min = min(lane_perc),
    covg_sd = sd(lane_perc)
  )
df_sum
```

```{r}
df_sum <- df %>%
  summarize(
    covg = mean(lane_perc),
    covg_max = max(lane_perc),
    covg_min = min(lane_perc),
    covg_sd = sd(lane_perc),
    speed_avg = mean(average_speed),
    speed_med = median(average_speed)
  )
df_sum
```

```{r}
# full summary for selected parameters
s <- fit4$summary()

# convert to data frame and keep rownames
tab <- as.data.frame(s)
tab$parameter <- rownames(s)
rownames(tab) <- NULL

tab_nor <- subset(tab, !grepl("^beta_r", parameter))

write.csv(tab_nor, "fit4_summary.csv", row.names = FALSE)
```

```{r}
# full summary for selected parameters
s <- fit5$summary()

# convert to data frame and keep rownames
tab <- as.data.frame(s)
tab$parameter <- rownames(s)
rownames(tab) <- NULL

tab_nor <- subset(tab, !grepl("^beta_r", parameter))

write.csv(tab_nor, "fit5_summary.csv", row.names = FALSE)
```

```{r}
# full summary for selected parameters
s <- fit4_2$summary()

# convert to data frame and keep rownames
tab <- as.data.frame(s)
tab$parameter <- rownames(s)
rownames(tab) <- NULL

tab_nor <- subset(tab, !grepl("^beta_r", parameter))

write.csv(tab_nor, "fit4_2_summary.csv", row.names = FALSE)
```
